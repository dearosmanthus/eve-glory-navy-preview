...
namespace EveOPreview.Services
{
	sealed class ThumbnailManager : IThumbnailManager
	{
		...
		public ThumbnailManager(IMediator mediator, IThumbnailConfiguration configuration, IProcessMonitor processMonitor, IWindowManager windowManager, IThumbnailViewFactory factory)
		{
			...
			RegisterQuickSwitchHotkeys();
			RegisterQuickHideHotkey();
...
*/
		...
		private void RegisterQuickSwitchHotkeys()
		{
			if (!this._configuration.QuickSwitchEnabled)
			{
				return;
			}

			var allClients = new Dictionary<string, int>();
			RegisterCycleClientHotkey(
				new[] { this._configuration.StringToKey(this._configuration.QuickSwitchForwardHotkey) },
				true,
				allClients);
			RegisterCycleClientHotkey(
				new[] { this._configuration.StringToKey(this._configuration.QuickSwitchBackwardHotkey) },
				false,
				allClients);
		}

		private void RegisterQuickHideHotkey()
		{
			if (!this._configuration.QuickHideEnabled)
			{
				this._quickHideActive = false;
				return;
			}

			Keys hotkey = this._configuration.StringToKey(this._configuration.QuickHideHotkey);
			if (hotkey == Keys.None)
			{
				return;
			}

			var handler = new HotkeyHandler(default(IntPtr), hotkey);
			handler.Pressed += (object sender, HandledEventArgs e) =>
			{
				this._quickHideActive = !this._quickHideActive;
				this.RefreshThumbnails();
				e.Handled = true;
			};
			handler.Register();
			this._cycleClientHotkeyHandlers.Add(handler);
		}
		...
		private void ThumbnailUpdateTimerTick(object sender, EventArgs e)
		{
			...
			this.RestoreMinimizedClientsInBackground();
			this.RefreshThumbnails();
		}

		private void RestoreMinimizedClientsInBackground()
		{
			...
			foreach (IThumbnailView view in this._thumbnailViews.Values)
			{
				...
				{
					this._windowManager.RestoreWindowInBackground(view.Id);
				}
			}
		}
		...
		private void RefreshThumbnails()
		{
			...
			bool hideOnLostFocus = this._configuration.HideThumbnailsOnLostFocus && !(isClientWindow || isMainWindowActive);
			bool hideAllThumbnails = this._quickHideActive || hideOnLostFocus;

			// Wait for some time before hiding all previews
			if (hideOnLostFocus && !this._quickHideActive)
			{
		...
		public void UpdateThumbnailsSize()
		{
			...
			foreach (IThumbnailView view in this._thumbnailViews.Values)
			{
				view.ThumbnailSize = this._configuration.GetThumbnailSize(
					view.Title,
					this._activeClient.Title,
		...
		private void SwitchActiveClient(IntPtr foregroundClientHandle, string foregroundClientTitle)
		{
			...
			// Minimize the currently active client if needed
			if (this._configuration.MinimizeInactiveClients && !this._configuration.KeepMinimizedClientsLive && !this._configuration.IsPriorityClient(this._activeClient.Title))
			{
		...
		private async void ThumbnailViewResized(IntPtr id)
		{
			...
			this._configuration.PerClientThumbnailSize[view.Title] = view.ThumbnailSize;
			view.Refresh(false);
