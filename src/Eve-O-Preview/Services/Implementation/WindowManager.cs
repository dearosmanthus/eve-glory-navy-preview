...
namespace EveOPreview.Services.Implementation
{
	public class WindowManager : IWindowManager
	{
		...
		public void RestoreWindowInBackground(IntPtr handle)
		{
			if (!this.IsWindowMinimized(handle))
			{
				return;
			}

			// DWM thumbnails stop receiving new frames while their source is truly
			// minimized. Restore without activation and place the client at the bottom
			// of the Z order so its preview can continue updating without stealing focus.
			User32NativeMethods.ShowWindowAsync(handle, InteropConstants.SW_SHOWNOACTIVATE);
			User32NativeMethods.SetWindowPos(
				handle,
				InteropConstants.HWND_BOTTOM,
				0,
				0,
				0,
				0,
				InteropConstants.SWP_NOMOVE | InteropConstants.SWP_NOSIZE | InteropConstants.SWP_NOACTIVATE);
		}
